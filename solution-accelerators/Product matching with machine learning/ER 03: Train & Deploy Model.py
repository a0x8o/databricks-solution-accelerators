...
# MAGIC %md The purpose of this notebook is use the product pairs assembled in the prior notebook to train and deploy a model capable of identifying potential product matches between two retailers.  This notebook has been developed using a Databricks ML 7.6 cluster.
...
labeled_pairs = (
  spark.table('DELTA.`/mnt/matching/silver/labeledpairs`')
  )
...
features = (
  spark.table('DELTA.`/mnt/matching/silver/abtbuyfeatures`')
  )
...
# DBTITLE 1,Get List of Experiments to Search
client = MlflowClient()
...
new_modified_products = spark.table('DELTA.`/mnt/matching/silver/abt`').sample(fraction=0.01).cache()
...
target = DeltaTable.forPath(spark, '/mnt/matching/silver/abtbuyfeatures')
(
  target.alias('target')
    .merge(
...
name_pairs = (
  lshmodels['name_lsher'].approxSimilarityJoin(
    ...
    lshmodels['name_lsher'].transform(
      spark.table('DELTA.`/mnt/matching/silver/abtbuyfeatures`').filter(f.expr("table='buy'"))
      ),
...
features = (
  pairs
    .join(spark.table('DELTA.`/mnt/matching/silver/abtbuyfeatures`').filter(f.expr("table='abt'")).alias('A'), on=f.expr('idA=A.id'))
    .join(spark.table('DELTA.`/mnt/matching/silver/abtbuyfeatures`').filter(f.expr("table='buy'")).alias('B'), on=f.expr('idB=B.id'))
    .withColumn('name_sqdist', f.expr('squared_dist(A.name_features, B.name_features)'))
    .withColumn('descript_sqdist', f.expr('squared_dist(A.descript_features, B.descript_features)'))
    .withColumn('price_sqdist', f.expr('squared_dist(A.price_features, B.price_features)'))

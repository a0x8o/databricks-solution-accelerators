...
# MAGIC %md The purpose of this notebook is access the data and generate the features used in later notebooks. This notebook was developed using a Databricks 7.6 ML cluster.
...
# MAGIC 
# MAGIC Having downloaded the data from the website and uploaded the unzipped data files to the appropriate folders under a [mount point](https://docs.databricks.com/data/databricks-file-system.html) named */mnt/matching*, we can make the data accessible as follows:
...
(
  spark
    .read
      .csv('/mnt/matching/bronze/Abt.csv', sep=',', header=True, schema=abt_schema)
      .withColumn('price', f.expr("cast(replace(price, '$', '') as float)"))  # strip characters from numerical strings and convert to float
    .write
      .format('delta')
      .mode('overwrite')
      .option('overwriteSchema', 'true')
      .save('/mnt/matching/silver/abt')
  )

# view data
display( spark.table('DELTA.`/mnt/matching/silver/abt`') )
...
(
  spark
    .read
      .csv('/mnt/matching/bronze/Buy.csv', sep=',', header=True, schema=buy_schema)
      .withColumn('price', f.expr("cast(replace(price, '$', '') as float)")) # strip characters from numerical strings and convert to float
    .write
      .format('delta')
      .mode('overwrite')
      .option('overwriteSchema', 'true')
      .save('/mnt/matching/silver/buy')
  )

# view data
display( spark.table('DELTA.`/mnt/matching/silver/buy`') )
...
(
  spark
    .read
      .csv('/mnt/matching/bronze/abt_buy_perfectMapping.csv', sep=',', header=True, schema=abtbuy_matched_schema)
    .write
      .format('delta')
      .mode('overwrite')
      .option('overwriteSchema', 'true')
      .save('/mnt/matching/silver/abtbuy_matched')
  )

# view data
display( spark.table('DELTA.`/mnt/matching/silver/abtbuy_matched`') )
...
abt = (
  spark
    .table('DELTA.`/mnt/matching/silver/abt`')
...
buy = (
  spark
    .table('DELTA.`/mnt/matching/silver/buy`')
...
(
  features
    ...
    .mode('overwrite')
    .option('overwriteSchema','true')
    .save('/mnt/matching/silver/abtbuyfeatures')
    )
...
# MAGIC %md
# MAGIC 
# MAGIC &copy; 2021 Databricks, Inc. All rights reserved. The source in this notebook is provided subject to the [Databricks License](https://databricks.com/db-license-source). 

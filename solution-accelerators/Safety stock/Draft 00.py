...
if not ('-ml' in spark.conf.get('spark.databricks.clusterUsageTags.sparkVersion')):
  ...
  dbutils.library.restartPython()
    ...
    raise Exception('If using a Databricks ML runtime, please install the FBProphet library using the steps documented here:\nhttps://docs.databricks.com/libraries.html#install-a-library-on-a-cluster')
...
result_schema =StructType([
  ...
  StructField('item',IntegerType()),
  StructField('date',TimestampType()),
  StructField('sales', FloatType()),             # historical sales
  StructField('sales_pred_mean', FloatType()),   # forecasted sales
  StructField('sales_pred_lower', FloatType()),
  StructField('sales_pred_upper', FloatType())
  ])
...
def get_forecast(keys, history_pd):
  ...
  # the interval width is being set to 0.95 in anticipation of later analysis
  model = Prophet(
    interval_width=0.95,
    growth='linear',
...
eval_schema =StructType([
  StructField('forecast_date', TimestampType()),
  StructField('store', IntegerType()),
  StructField('item', IntegerType()),
...
# MAGIC 
# MAGIC NOTE A formal validation that variability in demand is normally distributed around the predicted mean is beyond the scope of this particular blog but one we intend to address in future content.  Hopefully, the visual presentation gives enough confidence of the normal assumption for this analysis.

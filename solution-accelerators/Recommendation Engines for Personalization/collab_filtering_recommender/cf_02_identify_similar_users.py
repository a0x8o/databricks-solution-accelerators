...
# MAGIC %md The purpose of this notebook is to explore how similar users may be efficiently identified in preparation for the construction of a user-based collaborative filter. This notebook is designed to run on a **Databricks 7.1+ cluster**.  

# COMMAND ----------

# MAGIC %md # Introduction
# MAGIC 
# MAGIC Collaborative filters are an important enabler of modern recommendation experiences. ***Customers like you also bought***-type recommendations provide us an important means of identifying products that are likely to be of interest, expanding the range of products purchased or enabling the faster filling of customer's shopping carts:
# MAGIC 
# MAGIC <img src="https://brysmiwasb.blob.core.windows.net/demos/images/instacart_collabrecom.png" width="600">
# MAGIC 
# MAGIC With the transactional data required to build this kind of recommender in place, let's turn to core mechanism behind this type of filter which center on identifying customers with similar patterns of product purchases:
# MAGIC 
...
# MAGIC %md # Step 1: Explore Ratings Dataset
# MAGIC 
# MAGIC Before diving into the generation of a user-based collaborative filter (CF), let's first get oriented to our dataset by considering the number of customers for which we need to produce recommendations: 

# COMMAND ----------

# DBTITLE 1,Customers Requiring Recommendations
# MAGIC %sql
# MAGIC 
